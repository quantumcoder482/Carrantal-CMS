...
Class Dashboard{
    ...
    public static function dataLastTwelveMonthsIncExp(){
        ...
        foreach($months as $month){
            ...
            $inc[] = ORM::for_table('sys_transactions')->where('type','Income')->where_gte('date',$first_day_this_month)->where_lte('date',$last_day_this_month)->sum('cr');
            if($inc[$i] == ''){
                $inc[$i] = '0.00';
            }
        ...
        foreach($months as $month){
            ...
            $exp[] = ORM::for_table('sys_transactions')->where('type','Expense')->where_gte('date',$first_day_this_month)->where_lte('date',$last_day_this_month)->sum('dr');
            if($exp[$i] == ''){
                $exp[$i] = '0.00';
            }
